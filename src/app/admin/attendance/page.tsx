...
export default function AttendancePage() {
  ...
  const getPunctualityColor = (punctuality: string) => {
    switch (punctuality) {
      case 'EARLY': return 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400';
      case 'ON_TIME': return 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400';
            ...
            <table className="w-full text-left min-w-[800px]">
              <thead className="bg-gray-50 dark:bg-zinc-800/50">
                <tr>
                  ...
                  <th className="p-4 text-left text-sm font-semibold text-gray-600 dark:text-gray-300">Punctuality</th>
                ...
                {logs.length > 0 ? (
                  logs.map(log => (
                    <tr key={log.id} className="hover:bg-gray-50 dark:hover:bg-zinc-800/50 transition">
                      ...
                      <td className="p-4 text-left align-middle">
                        <span className={`px-2 py-1 rounded text-xs font-medium ${getPunctualityColor(log.punctuality)}`}>
